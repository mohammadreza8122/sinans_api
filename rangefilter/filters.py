...
from django.contrib.admin.widgets import AdminSplitDateTime as BaseAdminSplitDateTime,AdminDateWidget
...
class DateRangeFilter(BaseRangeFilter):
    ...
    def _make_query_filter(self, request, validated_data):
        ...
        import jdatetime
        valid_gte = str(validated_data.get(self.lookup_kwarg_gte, None)).split("-")
        valid_lte = str(validated_data.get(self.lookup_kwarg_lte, None)).split("-")
        
        try:

            date_value_lte = jdatetime.date(year=int(valid_lte[0]),month=int(valid_lte[1]),day=int(valid_lte[2]))
            date_value_gte = jdatetime.date(year=int(valid_gte[0]),month=int(valid_gte[1]),day=int(valid_gte[2]))
            converted_date_gte = jdatetime.date.togregorian(date_value_gte)
            converted_date_lte = jdatetime.date.togregorian(date_value_lte)

        except:
            converted_date_lte = date_value_lte
            converted_date_gte = date_value_gte
        if date_value_gte:
            query_params["{0}__gte".format(self.field_path)] = self.make_dt_aware(
                datetime.datetime.combine(converted_date_gte, datetime.time.min),
                self.get_timezone(request),
            )
        if date_value_lte:
            query_params["{0}__lte".format(self.field_path)] = self.make_dt_aware(
                datetime.datetime.combine(converted_date_lte, datetime.time.max),
                self.get_timezone(request),
            )
        ...
        return OrderedDict(
            (
                (
                    ...
                    forms.DateField(
                        label="",
                        widget=AdminJalaliDateWidget(attrs={"placeholder": _("From date")}),
                        localize=True,
                        required=False,
                ...
                (
                    ...
                    forms.DateField(
                        label="",
                        widget=AdminJalaliDateWidget(attrs={"placeholder": _("To date")}),
                        localize=True,
                        required=False,
