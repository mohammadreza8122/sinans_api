...
def order():
    ...
    for order in orders:
        try:
            ...
            print("date crate day", order.order.date_created.day)
            print("order.order.due_date_time.day", order.order.due_date_time.day)
            print("pay token", order.order.pay_token)
            print("tamam")

            if order.order.due_date_time:
                if (order.order.date_created.day == order.order.due_date_time.day) and (
                    (now - localtime(order.order.date_created)) >= timedelta(minutes=1)
                    and (now - localtime(order.order.date_created)) < timedelta(minutes=20)
                ):
                    ...
                    if not order.sms_send_city_managers:
                        ...
                        for manager in city_managers:
                            if manager.allowed_categories.exists():
                                ...
                                if categories:
                                    print('orderrrrrrrrrrrrrrrrrrr', )
                                    print(manager.user.number)
